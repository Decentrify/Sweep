...
public final class Cyclon extends ComponentDefinition {
	...
	Handler<CyclonJoin> handleJoin = new Handler<CyclonJoin>() {
		public void handle(CyclonJoin event) {
			...
			if (insiders.size() == 0) {
				...
				// schedule shuffling
				SchedulePeriodicTimeout spt = new SchedulePeriodicTimeout(shufflePeriod,
						shufflePeriod);
	...
	private void initiateShuffle(int shuffleSize, Address randomPeer) {
		...
		ArrayList<PeerDescriptor> randomDescriptors = cache.selectToSendAtActive(shuffleSize - 1,
				randomPeer);
	...
	Handler<ShuffleRequest> handleShuffleRequest = new Handler<ShuffleRequest>() {
		public void handle(ShuffleRequest event) {
			...
			DescriptorBuffer toSendRandomBuffer = new DescriptorBuffer(self,
					cache.selectToSendAtPassive(receivedRandomBuffer.getSize(), peer));
	...
	Handler<ShuffleTimeout> handleShuffleTimeout = new Handler<ShuffleTimeout>() {
		public void handle(ShuffleTimeout event) {
			...
			Address deadNode = outstandingRandomShuffles.get(rTimeoutId);
			outstandingRandomShuffles.remove(rTimeoutId);
			cache.removeEntry(deadNode);
		}
	};
